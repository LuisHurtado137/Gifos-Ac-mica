...
export default function Main(){
    ...
    return( 
        <ThemeProvider className="main-container" theme={theme === "Light" ? lightTheme : darkTheme}>
            <StyledApp className="styled-container">
              <div className="main-header-container">
                   <div className="header-container">
                        ...
                        <button onClick={() => handleTheme()}  className="theme-button" ><span><h3 className="button-text">{theme === 'Light' ? 'Dark' : 'Light'} Mode </h3></span></button>
                   </div>
                <div>
                     <h1 className="search-title">¡Inspirate y busca los mejores <strong>GIFS</strong>!</h1>
                </div>    
              </div>                
                <Search/>
                <div className="results-title">
                  <h5 >
                    {state.error ? ERROR : (state.gifs.length > 0 ? (state.loadedGifs ? RESULT : <Loading/>) : SEARCH)}
                  </h5>
                </div>
                <Gifs/>
            </StyledApp>
